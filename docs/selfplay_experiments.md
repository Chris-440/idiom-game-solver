...
### 交叉验证：这些"单边赢"的位置是模型弱还是游戏本质？

对每类位置回测 Q-table vs Q-table（双方均用 Q 表对弈）：

| 位置类型 | QT-QT P0 赢 | QT-QT P1 赢 | 结论 |
|----------|------------|------------|------|
| Only P0 (455) | **93.8%** | 6.2% | 先手必胜，游戏本质 |
| Only P1 (151) | 8.6% | **91.4%** | 后手反杀，游戏本质 |

**结论: 85% 是游戏数学上限，不是模型瓶颈。**

```
理论上限计算:
  双边赢:    1394 位置 × 1.0 = 1394
  先手必胜:   427 位置 × 0.5 =  213.5  (只能赢先手)
  后手反杀:   151 位置 × 0.5 =   75.5  (只能赢后手)
  ─────────────────────────────────────
  最大可得分: 1683 / 2000 = 0.841
...
不引入任何外部对手（Q-table、人类数据），仅靠同一模型双方对弈，模型从随机水平（0.3% vs Q-table）成长到 85%，接近游戏理论上限。
...
Q-table vs Q-table 自对弈中先手胜率 53.5%，模型 vs Q-table 中先手 92.4% vs 后手 77.2%。约 22% 的起始成语存在"先手必胜"的数学性质，无法通过改进策略消除。

### 5. 要进一步突破需要搜索

纯 NN 自博弈已经提取了策略空间的绝大部分信息。要突破游戏理论上限，需要在推理时引入树搜索（如 MCTS），类似 AlphaZero 的做法。
