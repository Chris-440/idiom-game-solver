...
成语接龙作为一种经典的中文语言博弈游戏，其策略分析长期停留在启发式规则层面。本文基于包含29,502个四字成语的数据集构建有向图模型，并通过迭代移除死端节点得到包含26,108个节点的实验图。带已使用集合的完整状态空间呈指数增长，因此本文研究两种近似框架：表格型Q-Learning与基于PPO的神经网络策略。Q-Learning通过500万局多核并行训练，对随机策略胜率约为97%；PPO通过纯自博弈训练，对固定Q-table基准的胜率稳定在约84%–86%。起点级分析显示性能与先后手角色高度相关，但现有结果属于策略对策略的经验评估，不构成全图minimax最优性的证明。
...
成语接龙（Idiom Solitaire）是汉语文化中广泛流传的语言博弈游戏。本文实验采用严格的“尾字等于首字”规则，且同一局中成语不可重复；同音接龙不在当前大规模RL实验范围内。该游戏可表示为有限状态空间上的完美信息零和博弈。

尽管规则简单，但由于成语库规模达到数万且图中包含大量环路，完整状态必须同时记录当前节点与已使用集合，传统精确求解面临状态空间爆炸。本文旨在通过图论分析与强化学习获得高性能近似策略，并明确区分经验基准性能与可证明的博弈论最优性。
...
为延长对局并减少由无后继节点造成的即时终止，采用迭代死端剪枝对图进行预处理。算法流程如下：
...
迭代剪枝共移除**3,394**个节点（约占原节点集的11.5%），最终保留**26,108**个节点。预处理后的子图具有以下特性：
...
直接枚举这一状态空间超出常规计算预算，说明朴素状态压缩动态规划无法扩展到全图；这并不排除利用额外图结构的精确算法。

### 4.3 实验验证

当前实现对200–300个节点的部分连通子图未能在预设计算预算内完成。该观察只反映实现与预算限制，不能作为一般不可解性证明；大规模实验因此采用近似策略。
...
![图3 起始位置分类分析](figures/research_matchup_results.png)

*图3 PPO与固定Q-table在2,000个起点上的确定性对局。69.7%的起点上PPO作为P0/P1均获胜，22.8%仅作为P0获胜，7.5%仅作为P1获胜。该分类描述策略对局结果，不是minimax局面分类。*
...
| 模型仅先手胜 | 455 | 22.8% | PPO作P0赢、作P1输 |
| 模型仅后手胜 | 151 | 7.5% | PPO作P0输、作P1赢 |
| 模型先手后手均负 | 0 | 0% | 无 |
...
为分析约85%的基准平台期，回测Q-table vs Q-table自对弈，并比较角色相关性。结果显示两个策略的对局结果具有相似的角色偏向，但Q-table本身是近似策略，因此该实验不能提供minimax证明。

![图5 角色分层基准结果](figures/research_matchup_results.png)

*图5 角色分层结果。该图报告PPO对固定Q-table的经验性能；策略对策略的胜负不能认证局面的真实minimax值。*

**表8 位置类型与Q-table自对弈结果**

| 位置类型 | QT-QT P0赢 | QT-QT P1赢 | 结论 |
|:---|:---:|:---:|:---|
| Only P0（455个） | 93.8% | 6.2% | 与P0结果高度相关 |
| Only P1（151个） | 8.6% | 91.4% | 与P1结果高度相关 |

实验结果表明PPO对当前固定Q-table形成稳定平台期，且角色差异不是单次评估噪声。要判断平台期来自游戏结构、策略共同偏差还是模型容量，需要在精确可解子图上计算真实值，并加入多种独立对手和best-response近似评估。
...
![图4 训练过程](figures/research_training_results.png)

*图4 Q-Learning与PPO训练过程。两个面板使用不同对手，不能直接横向比较。*

1. **Q-Learning方法**：对所评估随机策略胜率达97.4%，表明表格型强化学习在当前有限离散状态表示下形成了较强基线。状态空间压缩（忽略$U_t$）引入了信息损失，其影响仍需在精确可解子图上量化。

2. **PPO神经网络方法**：对固定Q-table基准胜率稳定在约84%–86%。Cross-Attention编码最近64步历史，环境使用完整used mask保证动作合法；当前证据支持较强的相对基准性能，不支持minimax最优性结论。
...
从博弈论角度，成语接龙与地理博弈（Geography）具有相似性。Schaefer[15]证明有向地理博弈是PSPACE完备的，这提示相关图博弈可能具有较高的最坏情形复杂度；但本文没有给出从一般实例到当前成语图的复杂性归约。
...
1. **拓扑预处理显著改变了实验环境**：移除3,394个迭代死端节点后，得到26,108个节点的实验子图；随机对局的平均长度由约5步增至237步。

2. **朴素精确求解不可扩展**：状压DP方法的复杂度为$O(|V| \cdot 2^{|V|})$；在全图规模下无法直接枚举，但这不等同于证明所有精确算法均不可行。

3. **Q-Learning形成了较强经验基线**：通过状态压缩（忽略$U_t$）和大规模自对抗训练，模型对所评估随机策略的胜率为97.4%；该结果仅说明在当前评估设置下有效。
...
1. **蒙特卡洛树搜索（MCTS）**：在神经网络策略引导下进行实时搜索，检验并纠正静态策略的局部偏差，并在精确可解子图上测量regret。
